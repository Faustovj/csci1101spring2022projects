...
plt.scatter(df_main["first_release_date"], df_main["user_rating_value"], color = "red", label = "User Rating")
...
df_follow = df_main.groupby(["genre_name"])
["follow_count"].sum().reset_index()

df_follow = df_follow.rename(columns = {"follow_count": "total_follows"})

df_hype = df_main.groupby(["genre_name"])
["hype_count"].sum().reset_index()

df_hype = df_follow.rename(columns = {"hype_count": "total_hypes"})

plt.bar(df_follow.index - BAR_WIDTH / 2, df_follow["total_follows"], color = "blue", label = "Number of Follows", width = BAR_WIDTH)

plt.bar(df_hype.index + BAR_WIDTH / 2, df_follow["total_hypes"], color = "red", label = "Number of Hypes", width = BAR_WIDTH)
...
df_hype = df_main.groupby(["platform_name"])
["hype_count"].sum().reset_index()
...
plt.bar(df_hype.index + BAR_WIDTH / 2, df_follow["total_hyoe"], color = "red", label = "Number of Hypes", width = BAR_WIDTH)
